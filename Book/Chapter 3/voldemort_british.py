...
def cv_map_filter(name, filtered_cv_map):
    """Remove permutations of words based on unlikely cons_vowel combos."""
    perms = {''.join(i) for i in permutations(name)}
    print("length of initian permutation set = {}".format(len(perms)))
    vowels = 'aeiouy'
    filter_1 = set()
...
def letter_pair_filter(filter_2):
    """Remove unlikely letter_pairs from permutations."""
    filtered = set()
...
def view_by_letter(name, filter_3):
    """Filter to anagrams starting with the input letter."""
