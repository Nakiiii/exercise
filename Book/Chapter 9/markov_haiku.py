...
os.chdir("d:\\exercise\\Book\\Chapter 9")
...
def random_word(corpus):
    """Return random word and syllable count form training corpus."""
...
def word_after_single(prefix, suffix_map_1, current_syls, target_syls):
    """Retrun all acceptable words in a corpus that follow a single word."""
    accepted_words = []
...
def haiku_line(suffix_map_1, suffix_map_2, corpus, end_prev_line, target_syls):
    ...
    while True:
        ...
        while len(word_choices) == 0:
            ...
            logging.debug("new random prefix = %s %s", prefix)
            word_choices = word_after_double(prefix, suffix_map_2,
                                             line_syls, target_syls)
        word = random.choice(word_choices)
        num_syls = count_syllables(word)
...
def main():
    ...
    intro = """\n
    A thousand monkeys at a thousand typewritters...
